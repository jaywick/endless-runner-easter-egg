...
(function() {
	...
	var setup = function () {
		...
		canvas.width = 700;
		canvas.height = 300;
		canvas.onclick = function (e) { tryJump(); e.stopPropagation(); };
		canvas.style.backgroundColor = "whitesmoke";
		canvas.style.border = "lightgray solid 1px"
		canvas.style.cursor = "pointer";
		canvas.title = "Click or Space to jump";
		...
		container.style.backgroundColor = "rgba(5, 5, 5, 0.5)";
		container.onclick = exit;
		container.appendChild(canvas);
		container.title = "Click or Escape to exit";
	...
	var exit = function () {
		document.removeEventListener("click", tryJump);
	...
	var createWave = function () {
		var side = 40;
		var minimumDistance = side * 3;
		var maximumDistance = side * 10;
		var obstacleCount = Math.ceil(Math.random() * wave);

		for (var i = 0; i < obstacleCount; i++) {
			var distance = (maximumDistance - minimumDistance) * Math.random() + minimumDistance;

			obstacles.push({
				x: canvas.width + (i * distance),
				y: 0,
				h: side,
				w: side,
				sprite: 0,
	...
	var createStars = function () {
		var starsPerElapsed = Math.pow(elapsed, 0.5) + 1;

		for (var i = 0; i < starsPerElapsed; i++) {
			stars.push({
